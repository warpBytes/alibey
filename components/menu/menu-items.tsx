...
const MenuItems = forwardRef<HTMLDivElement, { activeFilter: string }>(
  ({ activeFilter }, ref) => {
    const menuItems = Object.entries(
      menu[activeFilter as keyof typeof menu],
    ).map(([key, value]) => ({
      id: key,
      items: [...value],
    }));
    ...
    return (
      <div
        ref={ref}
        className="mx-auto flex w-full max-w-[950px] flex-col gap-20"
      >
